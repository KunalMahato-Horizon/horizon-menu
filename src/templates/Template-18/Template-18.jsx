...
const TemplateEighteen = ({ menuData }) => {
  ...
  return (
    <div className="min-h-screen bg-[#FAF5EC] text-[#19181A] font-sans antialiased selection:bg-[#D9822B] selection:text-white">
      <Header business={business} />
      <Hero business={business} />

      {/* Sticky Vinyl Track Nav */}
      <CategoryNav
      ...
      <section id="menu-root" className="max-w-5xl mx-auto px-4 sm:px-6 pt-2 pb-4">
        <div className="p-2 rounded-2xl bg-[#EDE5D6] border border-[#E0D5C3] flex flex-col sm:flex-row items-center gap-3">
          {/* Search Box */}
          <div className="relative w-full flex-1">
            <svg
              className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#4E4B45]"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <input
              type="text"
              placeholder="Search tracks, cuts, sips..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2 bg-white rounded-xl text-xs sm:text-sm text-[#19181A] placeholder-[#757169] border border-transparent focus:border-[#D9822B] focus:outline-none transition-all font-mono"
            />
          </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0 self-start sm:self-auto">
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-mono font-bold transition-all ${
                dietaryFilter === 'all'
                  ? 'bg-[#19181A] text-[#FAF5EC]'
                  : 'text-[#4E4B45] hover:bg-white'
              }`}
            >
              All Tracks
            </button>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 pt-4 pb-12">
        {filteredCategories.length > 0 ? (
          ...
          <div className="py-16 text-center text-[#757169] font-mono text-xs uppercase">
            No tracks found matching your current playback query.
          </div>
        )}
      </main>
