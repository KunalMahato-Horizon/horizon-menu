...
const TemplateNineteen = ({ menuData }) => {
  ...
  return (
    <div className="min-h-screen bg-[#0A0C10] text-white font-mono antialiased selection:bg-[#D4FF00] selection:text-[#0A0C10]">
      ...
      {/* Control Module (Search & Toggle Chips) */}
      <section id="menu-root" className="max-w-6xl mx-auto px-3 sm:px-6 pt-2 pb-4">
        <div className="p-2 rounded-2xl bg-[#121620] border border-white/10 flex flex-col sm:flex-row items-center gap-3">
          {/* Query Console */}
          <div className="relative w-full flex-1">
            ...
            <input
              type="text"
              placeholder="Filter docket items or ingredients..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2 bg-[#0A0C10] rounded-xl text-xs sm:text-sm text-white placeholder-white/30 border border-white/10 focus:border-[#D4FF00] focus:outline-none transition-all font-mono"
            />
          </div>

          {/* Hardware Dietary Switches */}
          <div className="flex items-center gap-1.5 shrink-0 self-start sm:self-auto">
            <button
              onClick={() => setDietaryFilter('all')}
              className={`px-3 py-1.5 rounded-xl text-xs font-mono font-bold transition-all ${
                dietaryFilter === 'all'
                  ? 'bg-[#D4FF00] text-[#0A0C10]'
      ...
      <main className="max-w-6xl mx-auto px-3 sm:px-6 pt-4 pb-12">
        {filteredCategories.length > 0 ? (
          ...
          <div className="py-20 text-center text-white/40 font-mono text-xs uppercase">
            // NO UNITS REGISTERED FOR CURRENT FILTER QUERY //
          </div>
        )}
      </main>
